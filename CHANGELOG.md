...
- The artifact home resolved against the working directory: a fresh one wherever
  you happened to be when you typed the command. That satisfies the one invariant
  ...
  mid-session. The workaround everyone reached for was pinning `<PREFIX>_DIR` to one
  ...
  is no checkout at all. `<PREFIX>_DIR` still wins outright, so a pinned setup is
  ...
  the artifact home beside it can no longer disagree about which checkout they are
  in.
