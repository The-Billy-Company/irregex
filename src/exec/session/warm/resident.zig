...
//! core (`kernel/query/query.zig`) — the SAME compile → trigram-prefilter → match
...
//!     / kqueue on macOS; `src/exec/session/watch/watch.zig`). This is the microsecond path.
...
//! (`kernel/math/lease.zig`) while a reconcile runs alone under the
...
const corpus = @import("mirror.zig");
...
// depending on `exec/cold` is a one-way edge (serial.zig never imports
// session), so no import cycle.
const run = @import("../../cold/engine/serial.zig");
const dirtylog = @import("../reconcile/dirty.zig");
const annalslog = @import("../reconcile/annals.zig");
...
pub const ResidentSession = struct {
    ...
    /// which the `Ward` (`kernel/math/lease.zig`) gathers into one
