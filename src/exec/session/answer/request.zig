...
//! glob, `--iglob`, `-t all`, or an unknown type declines. A positive genus
//! declines when any other positive family is present, because cold ORs them
//! where `PathFilter` ANDs. A scoped answer is a
...
// The corpus partition behind `--docs`/`--code`/`--data`, computed from the path
// by the same module cold consults — so a warm genus answer is cold's answer.
const genus = @import("../../../corpus/scope/genus.zig");

/// Eligible answer shapes — shared with the search core (`kernel/query/query.zig`).
pub const Mode = @import("../../../kernel/query/query.zig").Mode;
...
pub const Request = struct {
    ...
    /// `-w`/`--word-regexp` — see `kernel/query/query.zig::wordOk`.
    word: bool = false,
...
fn addGlob(inc: *Collector, exc: *Collector, raw: []const u8) ClassifyError!void {
    ...
    if (g.len == 0 or g[0] == '/' or glob.unterminatedClass(g)) return ClassifyError.Unsupported;
...
///
/// A GENUS name (`docs`/`code`/`data` and their aliases) is not a glob set at
/// all — it lands in the filter's own genus field, since `PathFilter` computes
/// that membership the same way cold does.
fn addType(ext: *Collector, gen: *genus.Set, name: []const u8) ClassifyError!void {
    if (name.len == 0 or std.mem.eql(u8, name, "all")) return ClassifyError.Unsupported;
    if (genus.named(name)) |g| return gen.merge(g);
    const globs = types.extsForType(name) orelse return ClassifyError.Unsupported;
    for (globs) |g| try ext.push(g);
}

/// Recognize a `--docs` / `--no-docs` / `--code` / … token. Enumerated from the
/// `Genus` members, so a new genus reaches both paths at once.
///
/// Matched against the CANONICAL label only, never `genus.named`'s aliases: the
/// aliases exist as `-t` type names, and there is no `--prose` flag row. Warm
/// must not accept a spelling cold would reject as an unknown flag — a fast path
/// may decline a valid request, but never admit an invalid one.
fn genusFlag(arg: []const u8) ?struct { set: genus.Set, negate: bool } {
    if (!std.mem.startsWith(u8, arg, "--")) return null;
    const body = arg[2..];
    const negate = std.mem.startsWith(u8, body, "no-");
    const name = if (negate) body[3..] else body;
    inline for (comptime std.enums.values(genus.Genus)) |g| {
        if (std.mem.eql(u8, name, @tagName(g))) return .{ .set = .one(g), .negate = negate };
    }
    return null;
}
...
pub fn classify(argv: []const []const u8, sa: *ScopeArgs) ClassifyError!Request {
    ...
    var genera: genus.Set = .empty;
    var neg_genera: genus.Set = .empty;

    var i: usize = 0;
    while (i < argv.len) : (i += 1) {
        const arg = argv[i];
        if (arg.len == 0) return ClassifyError.Unsupported;
        // Resolved before the chain because a bool cannot be `and`-ed with an
        // optional, and the genus arm needs both answers.
        const picked_genus = if (end_of_flags) null else genusFlag(arg);
        if (!end_of_flags and std.mem.eql(u8, arg, "--")) {
            ...
            i += 1;
            if (i >= argv.len) return ClassifyError.Unsupported;
            try addType(&exts, &genera, argv[i]);
        } else if (!end_of_flags and std.mem.startsWith(u8, arg, "--type=")) {
            try addType(&exts, &genera, arg["--type=".len..]);
            // `--docs`/`--no-docs`/`--code`/… — the fixed-name genus spellings.
            // `-T <genus>` stays cold with the rest of `-T`, so only the `--no-`
            // form reaches the warm negative set.
        } else if (picked_genus) |picked| {
            (if (picked.negate) &neg_genera else &genera).merge(picked.set);
        } else if (!end_of_flags and arg.len > 2 and std.mem.startsWith(u8, arg, "-t")) {
            try addType(&exts, &genera, arg[2..]); // glued `-tTYPE`
    ...
    if (rank_k != null and (fixed or word or pcre or invert or quiet or line_num or max_count != null or before != 0 or after != 0 or m != .lines or includes.n != 0 or excludes.n != 0 or exts.n != 0 or genera.any() or neg_genera.any()))
        return ClassifyError.Unsupported;
    // Cold ORs a genus selection with `-t`/`-g` includes; `PathFilter` ANDs its
    // constraint sets. With exactly one positive family present the two agree,
    // so a genus rides warm alone and declines the moment another one appears.
    // (A negative genus is a veto on both paths, so it composes freely.)
    if (genera.any() and (exts.n != 0 or includes.n != 0)) return ClassifyError.Unsupported;
