...
The cold search path. This is what backs bare `gist <pattern>`, `gist rg`,
and `gist search`: a byte-for-byte ripgrep-DEFAULT drop-in over an arbitrary
tree (gitignore precedence, piped stdin, coloring, exit codes), with the
persisted trigram index used only as a read-elision accelerator when it
covers the searched roots.
...
- **swarm** is the fused work-stealing walk+read+match, taken when the flag
  set allows.
- **ranked** is the `--rank` definition-first view, gist's one native shape.
...
- **[`engine/`](engine)** owns the scheduling that drives the five packages
  above.
- **[`view/`](view)** owns gist's own ways of looking at a match, the
...
which is exactly what keeps the parity certificate meaningful as gist grows
native shapes.
...
rgsuite certificate (`gist/bench/conformance/rgsuite/`, in the sibling
`gist` repo) is the parity gate for this face.
