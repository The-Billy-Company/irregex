...
pub const Queue = struct {
    ...
    // Directories fully listed, cumulative — the one number a walk that is
    // taking long enough to look broken can report about itself (`hints.Vigil`).
    // Bumped inside `done`, which already performs an atomic RMW on `live`, and
    // relaxed because nothing orders against it: a progress notice wants an
    // order of magnitude, not a synchronized snapshot. `live` stays the
    // outstanding front, so the two together say how far the walk has come AND
    // how much of it is still ahead.
    walked: std.atomic.Value(usize) = .init(0),
    ...
    pub fn done(q: *Queue) void {
        _ = q.walked.fetchAdd(1, .monotonic);
