...
| _(retrieval)_     | moved to `relate/src/exec/retrieval/` — fingerprint-lexicon path for `similar` / `pack`, shared by cold and warm. |
...
different algorithms on purpose (the cold-engine deep-module split) —
...
**Why relate's engine lives beside gist's.** the cold-engine deep-module split lifted the native lenses to
