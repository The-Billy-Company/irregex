...
- **[`cold/`](cold)** is transport rung 1, a fresh subprocess per query — the
  ...
  engine, and emit.
- **`relate/src/exec/retrieval/`** is a shared rung with no transport of its
  own: fingerprint-lexicon retrieval that `relate similar` / `pack` ride,
  used by both cold and warm.
...
[`session/README.md`](session/README.md), and `retrieval/README.md` in the
sibling `relate` repo's `src/exec/retrieval/`.
