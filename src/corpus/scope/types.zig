...
/// Which rows a listing may show, asked by canonical name. Null admits every
/// row, which is the rg-parity listing. The registry stays ignorant of what the
/// caller is partitioning by — `genus.zig` is built on top of this table, so
/// teaching the table to ask it back would invert the dependency.
pub const RowAdmits = *const fn (canonical: []const u8) bool;
...
pub fn writeTypeList(
    ...
    for (type_table) |row| {
        // A row is admitted or refused whole, under its canonical name, so every
        // alias travels with it: `--type-list --docs` keeps `md` with `markdown`.
        if (admits) |ok| if (!ok(row.names[0])) continue;
        for (row.names) |name| {
    ...
    for (overlay.added) |add| {
        ...
        // Judged by the same predicate as a table row, so a listing cannot be
        // narrowed and then have an overlay smuggle the refused name back in.
        if (admits) |ok| if (!ok(add.name)) continue;
