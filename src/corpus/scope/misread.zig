...
//! (`surface/exec/cold/argv/preference.zig`) are different formats for
...
/// drifted from the live list would stop testing that case. Imported in test
/// scope only, so the module itself stays a leaf both layers can depend on.
const charter_keys = @import("charter.zig").keys;

test "a near-miss is guessed at, a genuinely foreign name is not" {
    try t.expectEqualStrings("roots", nearest("root", &charter_keys).?);
    try t.expectEqualStrings("roots", nearest("rootz", &charter_keys).?);
    try t.expectEqualStrings("types", nearest("type", &charter_keys).?);
    try t.expectEqualStrings("skip", nearest("skips", &charter_keys).?);
    ...
    for ([_][]const u8{ "generated", "exclude", "include", "ignore", "" }) |foreign| {
        try t.expectEqual(@as(?[]const u8, null), nearest(foreign, &charter_keys));
    }
}
...
test "a transposition is one edit, not two" {
    ...
    try t.expectEqualStrings("roots", nearest("rotos", &charter_keys).?);
    try t.expectEqualStrings("types", nearest("tpyes", &charter_keys).?);
    try t.expectEqualStrings("heading", nearest("headnig", &.{ "heading", "hidden" }).?);
}
