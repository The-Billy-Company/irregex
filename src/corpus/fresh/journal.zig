...
//!
//! The cost of a replay tracks the width of its window in DEVICE events, not
//! the size of the corpus: this tree's box issues ~20M events in two hours, and
//! draining a two-hour window took 2.4 s against a 750 ms stat sweep, while a
//! seven-second window drained in 8.6 ms. A token pinned to build time
//! therefore decays from a large win into a guaranteed loss within the hour —
//! which the budget catches, at the price of never winning again. So an empty
//! replay re-dates the token (`Token.certified_ns`), keeping the window as
//! narrow as the gap between two queries.
//!
...
const magic = "GISTJRN2";
const magic_v1 = "GISTJRN1"; // pre-certification layout; still decodes (certified_ns = 0)
pub const encoded_len = 40; // magic(8) + event_id u64 + dev u64 + captured_ns i64 + certified_ns i64
const encoded_len_v1 = 32;
...
pub const Token = struct {
    ...
    /// sound; a newer one has a blind window — never) — OR when `certified_ns`
    /// closes that window, below.
    captured_ns: i128,
    /// The build anchor this token was PROVEN to carry no blind window for, or
    /// 0 for a plain build-time mint. A token minted after the anchor normally
    /// hides (anchor, captured_ns) from every replay; but a replay that came
    /// back EMPTY proves nothing changed in that span, so a token captured
    /// before that replay began may be re-dated against exactly that anchor and
    /// used as if it predated it. This is what keeps the replay window the size
    /// of the gap between two queries instead of the age of the build: on a
    /// busy tree the device issues millions of events an hour, and draining a
    /// build-old window costs seconds where the walk it replaces costs ~750 ms
    /// (measured). It must be compared for EQUALITY with the live anchor — a
    /// certification is a claim about one anchor, and a rebuild moves it.
    certified_ns: i128 = 0,
};
...
pub fn encode(t: Token) [encoded_len]u8 {
    ...
    std.mem.writeInt(i64, buf[32..40], @intCast(t.certified_ns), .little);
    return buf;
}

pub fn decode(bytes: []const u8) ?Token {
    // A v1 token predates certification, so it reads as uncertified — the
    // conservative value, which leaves it governed by the plain
    // `captured_ns <= anchor` rule it was written under.
    if (bytes.len >= encoded_len and std.mem.eql(u8, bytes[0..8], magic)) return .{
        ...
        .certified_ns = std.mem.readInt(i64, bytes[32..40], .little),
    };
    if (bytes.len >= encoded_len_v1 and std.mem.eql(u8, bytes[0..8], magic_v1)) return .{
        .event_id = std.mem.readInt(u64, bytes[8..16], .little),
        .dev = std.mem.readInt(u64, bytes[16..24], .little),
        .captured_ns = std.mem.readInt(i64, bytes[24..32], .little),
    };
...
/// The exclusion CANDIDATES, richest-first — `setExclusionPaths` caps at 8, so
/// this is a priority order over the noisiest non-corpus subtrees, not a policy.
/// The policy is `haystack.isSkipDir`, which every name here is filtered
/// through below.
const noisy_candidates = [_][]const u8{
    ".git",    ".local", ".zig-cache", "zig-out",       "node_modules",
    ".cursor", "target", "dist",       "__pycache__",   ".venv",
    ".next",   "build",  "vendor",     ".pytest_cache", ".cache",
};

/// Kernel-side exclusion of the noisiest non-corpus subtrees, and the one place
/// this accelerator could turn into a correctness bug: a subtree the daemon
/// never reports is a subtree whose changes this replay cannot see, so an
...
/// a refusal only costs replay volume, never exactness.
fn excludeNoise(s: *Syms, stream: Ref, prefixes: []const RootPrefix, aa: std.mem.Allocator) void {
    ...
    for (noisy_candidates) |n| {
        if (made == refs.len) break;
        if (!haystack.isSkipDir(n)) continue; // the walk WOULD descend here — never hide it
    ...
    if (made == 0) return;
    const arr = s.CFArrayCreate(null, refs[0..made].ptr, @intCast(made), s.array_callbacks) orelse return;
    defer s.CFRelease(arr);
    _ = s.FSEventStreamSetExclusionPaths(stream, arr);
}

test "every kernel-side exclusion candidate is a directory the walk already skips" {
