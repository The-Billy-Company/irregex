...
same corpus without per-machine folklore. Scope prunes **before** disk when the
path list is already known (faster than rg's walk-then-filter).

## When to edit

Skip-directory policy, ignore _shared_ helpers that aren't cold-specific,
bulkstat / portable stat parity, `-g` / `-t` tables, charter keys, the stdout
drain, persisted artifact formats (`index/`), or the output budget on loaded
corpora. rg-compatible ignore _dialect_ wiring for the cold face lives beside
the cold engine in `surface/exec/cold/`.
