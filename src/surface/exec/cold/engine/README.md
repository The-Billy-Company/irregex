...
doc_radar:
  sentinels:
    ...
    - description: "the fused pipeline is still reached through its eligibility test, so an ineligible flag set falls through to serial"
      file: pkg/kernels/irregex/src/surface/exec/cold/engine/swarm/swarm.zig
      contains: ["pub fn eligible", "pub fn run"]
    - description: "relate's cold recall engine stays the one non-gist scheduler this package hosts"
      file: pkg/kernels/irregex/src/surface/exec/cold/engine/retrieval.zig
      contains: ["pub fn"]
---
...
search. Matching itself lives in `kernel/match/query/query.zig`; the tree walk, read
...
| `retrieval.zig`   | relate's cold recall engine (a `relate similar <text>` probe / `pack`): nominate a bounded pool from the mmap trigram postings, fold the freshness overlay, price it with the `zipper` cross-parse. Reads the persisted index (never a walk+match), so it shares this tier with the gist engines rather than the pure `kernel/` kernels. |
