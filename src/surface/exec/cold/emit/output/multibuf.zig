...
pub fn buffer(self: *Emitter, path: []const u8, body: []const u8) usize {
    ...
    if (o.invert) return bufInvert(self, path, body);
    ...
    if (o.vimgrep) return bufVimgrep(self, path, lines, spans, body);
...
fn bufVimgrep(self: *Emitter, path: []const u8, lines: []const ml.Line, spans: []const ml.Span, body: []const u8) usize {
    ...
    for (spans) |sp| {
        ...
        display.emitBody(self, body[ln.start..ln.content_end], true, &.{sp.start - ln.start}, ln.term_end > ln.content_end);
        emitted += 1;
    }
    return emitted;
}
