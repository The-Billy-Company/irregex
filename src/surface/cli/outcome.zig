...
/// buffering policy and exits; a face that never installs anything loses no
/// correctness, only the keep.
pub var departure: *const fn (code: u8) noreturn = plainDeparture;

fn plainDeparture(code: u8) noreturn {
    corpus.flushStdout();
...
pub fn die(comptime msg: []const u8, args: anytype) noreturn {
    ...
    // an answer, so nothing about it should be kept); it settles the stdout
    // buffering policy itself: results already printed before the fault are
