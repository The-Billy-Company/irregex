...
// gist "pays tokens for every line of output": an unbounded result dump can blow
...
// ceilings — gist reasons in bytes, never tokens (it scans raw bytes and cannot
...
//     Once the emitted/accumulated total crosses it, gist stops producing
...
//     firehose.
//   • hard — the OOM ceiling. Always on, even under `--uncap`; the most gist will
