...
`match/` wrapper. A linear-time **Thompson NFA** over bytes (RE2 / ripgrep
philosophy) with a byte-class DFA as the primary O(1)/byte engine, a Pike VM
fallback, optional accelerator rungs, and opt-in PCRE2. Organized by real
pipeline stage — the tree flows front-to-back, `syntax → analysis / ast →
compile → linear`, with `unicode` data feeding the class lowering, `pcre2` the
opt-in escape hatch, `matcher.zig` the meta dispatcher, and `oracle` the
independent correctness backstop. The ambition is to beat rust-regex; the seam mirrors that
ecosystem (`regex/` ≈ regex-syntax + regex-automata; sibling `scan/` ≈ memchr +
aho-corasick + teddy; sibling `query/` ≈ the meta engine).

**One door in.** These eight stages are internals: every caller outside this
...
| linear   | [`linear/`](linear)     | The engine: the compiled handle (`program/`), engine selection (`ladder/`), the two machines that answer (`pike/`, `dfa/`), a second route to the same DFA table (`symbolic/`), and the optional rungs that beat it where they apply (`shuffle/`, `parabix/`, `sieve/`, `caliper/`). |
