...
//!    This is the failure mode the prior
...
///
/// **Arming is judged at the coarser one**, for two independent reasons that
/// happen to agree. First, a sieve serves both paths from one field, so it has
/// to pay at the harder grain or it is a loss on half its callers. Second, the
/// estimate feeding the test is known to be OPTIMISTIC — `fallthroughRate`
/// prices each position under a memoryless byte prior, and real text is not
/// memoryless: pattern-shaped bytes cluster, so one survivor drags a whole
/// buffer into verification. Measured on this corpus, the model was low by up
/// to five orders of magnitude (`uuid`: est 4.3e-7 vs 1.6e-2). Requiring the
/// bound to clear at 4 KiB rather than 64 B is how a structural estimate buys
/// margin against its own known bias without observing any traffic.
///
