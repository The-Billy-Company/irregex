...
/// moving together is what keeps `stride ≈ 1/P` — and so the `min_stride` bars
/// calibrated against it — invariant when the table is re-measured. 65535 also
/// makes a saturated `mass` the widest `u16`, so `Economics` stays two u16s.
const probability_scale: u32 = 65535;
...
/// Price a candidate-byte set against the checked-in corpus prior: the set's
/// probability mass is the sum of its members' cells, and the expected stride
/// between candidates is its reciprocal.
///
/// RECORDED DEFECT (2026-07-29): `density` used to saturate at 255, so a cell at
/// the ceiling meant "at least this common" rather than a probability, and this
/// function carried a sentinel branch expanding 255 to a guessed 2048 — a made-up
/// number standing in for the range the table had thrown away, and one that
/// priced the space (P = 17.7%) identically to `c` (P = 2.2%). The table now
/// carries its full range, so the sum is just a sum. Never reintroduce a
/// magic-value branch here: if a cell needs interpreting, the table is wrong.
///
/// Zero cells retain one unit so a byte the corpus never saw has a finite,
/// conservative stride rather than infinity.
pub fn estimate(set: ByteSet) Economics {
    var mass: u32 = 0;
    for (0..256) |i| if (set.has(@intCast(i))) {
        mass += @max(@as(u32, rarity.density[i]), 1);
    };
...
test "economics distinguishes a common byte from a rare set" {
    var common: ByteSet = .{};
    common.set(' '); // the corpus's single densest byte ⇒ no skip earned
    var rare: ByteSet = .{};
    // Genuinely rare in the checked-in density table (J=53, Q=36, Z=55 against
    // the space's 11576). NOT `_` (573), which is common in code identifiers —
    // a small set of common bytes is exactly the trap `estimate` prices out.
...
test "economics prices three ranks of commonness apart, with no cell at a ceiling" {
    // The clamp's signature failure: `_` and ` ` both sat at 255, so a set of
    // underscores was priced exactly as dense as a set of spaces, and both as
    // dense as `e`. Ordering — not the absolute mass — is what this table owes
    // its consumers, so assert the ORDER survives the sum.
    var space: ByteSet = .{};
    space.set(' ');
    var under: ByteSet = .{};
    under.set('_');
    var rare: ByteSet = .{};
    rare.set('Q');
    try std.testing.expect(estimate(space).mass > estimate(under).mass);
    try std.testing.expect(estimate(under).mass > estimate(rare).mass);
    try std.testing.expect(estimate(space).stride < estimate(under).stride);
    try std.testing.expect(estimate(under).stride < estimate(rare).stride);
}
