...
test "the trawl reports exactly what a substring search would" {
    const lits = [_][]const u8{ "refund", "wallet", "Service", "fund" };
    const owner = [_]u32{ 0, 1, 2, 3 };
    try expectPlay(&lits, &owner, 4, "pub fn handleRefund(w: *WalletService) void {}");
    try expectPlay(&lits, &owner, 4, "refund the wallet");
