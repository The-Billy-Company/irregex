...
test "declaration confidence follows geometry across syntax families" {
    ...
    try expect(confidence("func NewAcme(cc grpc.ClientConn) Acme {", "NewAcme") == 3);
    try expect(confidence("pub fn parse(self: *T) !void {", "parse") == 3);
    try expect(confidence("def charge(self, amount):", "charge") == 3);
    try expect(confidence("class AcmeStore:", "AcmeStore") == 3);
    try expect(confidence("function getAcme() {", "getAcme") == 3);
    try expect(confidence("type Acme struct {", "Acme") == 3);
    try expect(confidence("interface AcmeStore {", "AcmeStore") == 3);
    try expect(confidence("defmodule Acme do", "Acme") == 1); // word-delimited, punctuation-free declaration
...
test "declaration geometry rejects parameters, imports, uses, and fragments" {
    try expect(!signals.definesNeedle("    a := NewAcme(cc)", "NewAcme")); // call site
    try expect(!signals.definesNeedle("    return charge(amount)", "charge")); // call site
    try expect(!signals.definesNeedle("    var x = AcmeStore", "AcmeStore")); // RHS of =
    try expect(!signals.definesNeedle("    log(\"AcmeStore started\")", "AcmeStore")); // string
    try expect(!signals.definesNeedle("    fields := []T{Acme, Ledger}", "Acme")); // list element (comma)
    try expect(!signals.definesNeedle("type AcmeStoreImpl struct {", "AcmeStore")); // mid-identifier
...
test "isGenerated by ecosystem suffix" {
    const empty = "";
    try expect(signals.isGenerated("services/api/acme.pb.go", empty)); // Go protobuf
    try expect(signals.isGenerated("x/acme_pb2.py", empty)); // Python protobuf
    try expect(signals.isGenerated("x/acme_pb2_grpc.py", empty)); // Python grpc
    try expect(signals.isGenerated("x/Acme.pb.cc", empty)); // C++ protobuf
    try expect(signals.isGenerated("x/acme.g.dart", empty)); // Dart codegen
    try expect(signals.isGenerated("x/Acme.designer.cs", empty)); // C# designer
    try expect(signals.isGenerated("x/app.min.js", empty)); // minified
    try expect(!signals.isGenerated("services/api/acme.go", empty)); // hand-written
...
test "isGenerated finds a banner below a long inherited header comment" {
    // protoc-gen-connect-go carries the .proto's leading comment above its own
    // banner, so the marker lands well past the eight-line floor.
    const prose = "// contract prose describing the wire surface\n" ** 24;
    const banner = "\n// Code generated by protoc-gen-connect-go. DO NOT EDIT.\n\npackage x\n";
    try expect(signals.isGenerated("acme.pbconnect.go", prose ++ banner));

    // The header ends at the first body line — a marker under real code past
    // the floor is a body mention, not a banner.
    const body = "package x\n" ++ ("func f() {}\n" ** 12) ++
        "// Code generated by a tool. DO NOT EDIT.\n";
    try expect(!signals.isGenerated("a.go", body));
}

test "isGeneratedPath knows the buf Connect suffixes" {
    const empty = "";
    try expect(signals.isGeneratedPath("api/pb/acmepbconnect/acme.connect.go"));
    try expect(signals.isGenerated("api/pb/acmepbconnect/acme.connect.go", empty));
    try expect(signals.isGenerated("Shared/acme.connect.swift", empty));
    try expect(!signals.isGenerated("api/internal/db/connect.go", empty));
}
