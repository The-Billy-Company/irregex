...
test "isGenerated finds a banner below a long inherited header comment" {
    ...
    try expect(signals.isGenerated("wallet.pbconnect.go", prose ++ banner));
...
test "isGeneratedPath knows the buf Connect suffixes" {
    const empty = "";
    try expect(signals.isGeneratedPath("api/pb/walletpbconnect/wallet.connect.go"));
    try expect(signals.isGenerated("api/pb/walletpbconnect/wallet.connect.go", empty));
    try expect(signals.isGenerated("Shared/wallet.connect.swift", empty));
