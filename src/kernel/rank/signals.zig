...
const gen_exact_suffixes = [_][]const u8{
    ...
    ".connect.go", ".connect.swift",
};
/// Header markers — the universal generated signal, language-independent and
/// more reliable than filename inference. Only the header counts: the leading
/// run of comment and blank lines, plus a floor of eight lines for generators
/// that stamp their banner just under a package clause.
...
/// A blank line or one opening with a comment marker — the two things a file's
/// header is made of. Anything else is the first line of the body.
fn headerLine(line: []const u8) bool {
    const text = std.mem.trim(u8, line, " \t\r");
    if (text.len == 0) return true;
    for (comment_prefixes) |prefix| if (std.mem.startsWith(u8, text, prefix)) return true;
    return false;
}

fn generatedHeaderLine(line: []const u8) bool {
    var text = std.mem.trim(u8, line, " \t\r");
    for (comment_prefixes) |prefix| if (std.mem.startsWith(u8, text, prefix)) {
...
pub fn isGenerated(path: []const u8, buf: []const u8) bool {
    ...
    var seen: usize = 0;
    var in_header = true;
    // A generator that inherits its source contract's leading comment stamps
    // its banner well below line eight, so the header runs as long as the
    // comment block does; the eight-line floor keeps the older behavior for
    // banners that sit just under a package clause.
    while (in_header or seen < 8) : (seen += 1) {
        const line = lines.next() orelse break;
        if (generatedHeaderLine(line)) return true;
        in_header = in_header and headerLine(line);
    }
    return false;
}
