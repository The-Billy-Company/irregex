...
doc_radar:
  counts:
    - description: "match keeps the query core beside its regex + scan execution packages"
...
| Piece             | Job                                                                                                                                                           |
| ----------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| [`query/`](query) | `CompiledQuery` — the search intent itself, with the two derivations only it may make: the sound index-pruning `prefilter` and the rg `-w` word-boundary rule |
