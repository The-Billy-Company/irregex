...
test "onepass: eligibility census over a realistic -r/--json pattern corpus" {
    ...
    for (corpus) |p| {
        ...
        if (!ok[0] and ok[1]) std.debug.print("[onepass] unicode-only refusal: {s}\n", .{p});
    }
    std.debug.print("[onepass] eligibility census over {d} real patterns: unicode {d}/{d}, no-unicode {d}/{d}\n", .{ corpus.len, uni, corpus.len, ascii, corpus.len });
