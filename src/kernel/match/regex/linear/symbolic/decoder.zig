...
const WordsCtx = struct {
    pub fn hash(_: WordsCtx, k: []const u64) u64 {
        return std.hash.Wyhash.hash(0, std.mem.sliceAsBytes(k));
    }
    pub fn eql(_: WordsCtx, a: []const u64, b: []const u64) bool {
...
pub const Decoder = struct {
    ...
    pub fn follow(d: *const Decoder, node: u32, b: u8) ?u32 {
        for (d.edgesOf(node)) |e| if (e.lo <= b and b <= e.hi) return e.target;
        return null;
    }
