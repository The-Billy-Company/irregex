...
| File            | Role                                                                                                                                                                                                       |
| --------------- | ---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `core.zig`      | The public `Regex` handle: the immutable compiled state with its field-by-field contract, `deinit`, the program-walk predicates (`canMatchByte` / `bansByte`), and the decls adopted from every neighbor. |
| `lower.zig`     | Compilation: parse → case-fold → Thompson lowering → the accelerator analyses (required literal, alternation cover, pure literals, first set, zero-width reachability) → which engines to build.           |
