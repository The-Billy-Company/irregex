...
doc_radar:
  counts:
    - description: "math floor zig sources at package root (bits/crest/forest/glob/lease/misread/mix/parallel + tests)"
...
The lowest kernel tier: pure, product-free arithmetic. No walk policy, no CLI,
no regex opinion. Kept physically away from engine code so bit tricks and sieve
calculus cannot grow a dependency on a matcher.
...
structures. Never file I/O or ignore rules — that is `corpus/` / `exec/cold/`.
