...
//! `commands/ripgrep/run.zig::defaultFileSet`) into resident documents with the
...
pub fn load(gpa: std.mem.Allocator, io: std.Io, in_paths: []const []const u8) !Mirror {
    ...
    return .{ .docs = docs.items, .paths = paths.items, .nuls = nuls.items, .bytes = total, .arena = arena };
}
