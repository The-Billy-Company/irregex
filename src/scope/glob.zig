//! irregex glob matching + path scoping — the `rg -g <glob>` / positional-`PATH`
//! affordance an agent reaches for to confine a search to a subtree. This is the
//! one place irregex can be *faster* than rg rather than merely matching it: rg
//! applies a glob filter while walking the whole tree, but irregex already holds the
