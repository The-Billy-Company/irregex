...
doc_radar:
  sentinels:
    - description: "the public Regex handle + its DFA-primary / Pike-fallback dispatch live here"
      file: pkg/kernels/irregex/src/search/match/regex/linear/core.zig
    ...
    - description: "the engine-neutral seam forwards to the linear arm or the PCRE2 backend"
      file: pkg/kernels/irregex/src/search/match/regex/linear/matcher.zig
...
DFA / Pike dispatch, powerset `max_states` policy, or the `Matcher` seam to
PCRE2. Grammar changes → `../syntax/`; independent correctness cases →
`../oracle/`.
