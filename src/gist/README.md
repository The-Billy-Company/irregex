...
**Regex** (`src/gist/kernel/regex/`). A linear-time **Thompson NFA** over bytes (the
...
**Freshness overlay** (`src/gist/kernel/index/fresh.zig`). Keeps a persisted index correct
...
**Ranking** (`src/gist/kernel/rank/rank.zig`). Turns the verified match set into the list an agent
...
**Compiled query core** (`src/gist/kernel/engine/query.zig`). One deep module owns "a search
...
`dups` — ADR-363, [`src/primitives/`](../primitives/README.md)) live in the
...
([`src/gist/kernel/rank/rank.zig`](kernel/rank/rank.zig), Cormack et al. 2009) over four
...
([`src/gist/kernel/regex/unicode/`](kernel/regex/unicode)) woven into the same byte NFA, so
...
`pkg/kernels/irregex/src/gist/kernel/regex/unicode/tables.gen.zig`, `make gen-gist-unicode`),
...
these symbols — including the rung-3 session end to end (see `src/gist/faces/ffi/`).
