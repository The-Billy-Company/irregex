...
const Corpus = struct {
    ...
    /// Independent oracle: the abs paths of live files whose bytes contain
    /// `needle` on some line — a plain re-read + substring scan, no engine.
...
fn assertMatchesOracle(session: *ResidentSession, corpus: *Corpus, gpa: std.mem.Allocator, needle: []const u8) !void {
    ...
    var want_files: std.ArrayList([]const u8) = .empty;
    try corpus.oracleFiles(&want_files, needle);
    const want_count = try corpus.oracleCount(needle);

    const got_files = try session.query(qa, .{ .pattern = needle, .mode = .files, .fixed = true });
    try std.testing.expectEqual(want_files.items.len, got_files.files.len);
    const sorted = try qa.dupe([]const u8, got_files.files);
    std.mem.sort([]const u8, sorted, {}, lessPath);
    for (want_files.items, sorted) |w, g| try std.testing.expectEqualStrings(w, g);
