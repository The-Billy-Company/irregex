...
The Zig tree is the LIBRARY the ecosystem's three product faces (`gist` ·
`relate` · `blast`) stand on, organized into five layers, read bottom-up.
...
- [`kernel/`](kernel) is pure compute: no argv, walk, or emit. Its own
  ...
  same ward, `kinship` and `compose`, are zoned in the sibling `relate`
...
- [`surface/`](surface) is vocabulary plus transports: `cli/` · `api.zig` ·
  `ffi/`. The three product faces (`gist`/`relate`/`blast`) that used to
  live here now ship from their own sibling repos. See
...
index resident, riding this library's `exec/session/` machinery from the
`gist` package's own `session/daemon/`. Any failure falls back to the
...
- **`assay/`** provides typed clocks, counters, and `GIST_TRACE` — std-only
  so every tier can consume it.
...
- `relate/src/kernel/codex/` is the Ziv–Merhav cento quoter over this
  package's FM-index.
- `relate/src/kernel/compose/` runs set algebra over candidate sets.
...
- [`index/`](corpus/index) holds the persisted artifacts: trigrams ·
  ...
  atlas and frag artifacts are persisted by the `relate` package instead.

## `exec/` — The Two Runtimes (Plus Shared Retrieval)

- [`cold/`](exec/cold) is one process per query: argv → writ → quarry →
  read → engine → emit.
- `relate/src/exec/retrieval/` is fingerprint-lexicon retrieval shared by
  `similar` / `pack`.
- [`session/`](exec/session) is the warm resident session: answer · facet
  ...
  resident lives in the `gist` package's `session/daemon/`.
...
- **`ffi/`** is this library's own C-ABI plane: a pattern over a buffer the
  ...
  session-shaped ABI and the three product faces (`gist`/`relate`/`blast`)
  build on top of it from their own repos.
