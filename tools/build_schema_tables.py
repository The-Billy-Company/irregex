...
The analytic C ABI returns ONE self-describing row for all seventeen
verbs: a schema id, a presence mask, and a flat `irgx_value` array. What that
array MEANS lives in `contract/analytic.toml` — `[row_enums]`, `[row_schemas]`,
...
into every consumer, so a decoder is never hand-mirrored:
    ...
    src/surface/ffi/schema.gen.zig          the engine's own table

Every verb also carries the C symbol that ANSWERS it (`[analytic.producers]`),
because three libraries do now: `gist_run` for rank, `relate_run` for the
kinship/retrieval/sweep verbs, `blast_run` for the composed ones. The op numbers
stayed ecosystem-wide, so nothing about op 4 says which — a binding that guessed
from an op range would mis-route the next verb appended to a family.

The table is substrate: the row layout every product returns belongs to the
engine, not to whichever product happened to declare it first. `_zig` used to be
absent from TARGETS because the emitter and `[row_schemas]` lived in different
repositories, so neither could own the whole loop and the engine's table had to
be regenerated by hand from over there. Both are here now, so the loop closes and
`--check` covers it.

The per-language tables land under `bindings/` here too, for the same reason: a
decoder is substrate, so gist, relate and blast import one rather than each
generating a copy of the same rows:

    bindings/python/irgx/schema.gen.py   the Python decoder's table
    bindings/rust/src/schema.gen.rs         the Rust decoder's table
    bindings/go/analytic/schema_gen.go      the Go decoder's table

Every table carries the SAME digest. `irgx_schema_digest()` returns the
engine's; a binding compares it to its own generated constant at load, so a stale
shared library is a loud startup failure rather than a silently mis-decoded row.
The engine's table has no `entry`: the engine IS a producer, so routing is a
client-side fact and would be noise there.

stdlib-only and deterministic, so `--check` is a sound regenerate-and-diff drift
gate (CI-hermetic; no network), like the kernel's own table generators.

Run: python3 tools/build_schema_tables.py           # write
     python3 tools/build_schema_tables.py --check   # diff only
"""
...
ROOT = HERE.parent
CONTRACT = ROOT / "contract" / "analytic.toml"

# Value tags — the wire discriminant, mirrored in include/irgx.h. Order IS
...
class Verb:
    ...
    stream: str
    entry: str
    """The C symbol that answers this verb, resolved through `[analytic.producers]`.

    Carried per verb rather than derived from the op, because the op numbers are
    ecosystem-wide: nothing about op 4 says `relate_run` rather than `gist_run`
    answers it, and an op-range rule would mis-route the next verb appended to a
    family.
    """
...
def load(path: Path = CONTRACT) -> Table:
    ...
    raw_producers = doc["analytic"]["producers"]
...
def _digest(enums: tuple[Enum, ...], schemas: tuple[Schema, ...], verbs: tuple[Verb, ...]) -> str:
    ...
    A verb's `entry` symbol is deliberately NOT in it. The digest answers "can
    this decoder decode this library's rows", and which library was asked is not
    part of that — folding it in would turn a pure routing rename into a loud
    startup failure for a binding that decodes perfectly, which is the exact
    false positive the paragraph above exists to avoid.
    """
...
def _zig(t: Table) -> str:
    out = [
        f"//! Generated by tools/{BANNER} from contract/analytic.toml — DO NOT EDIT.",
        "//!",
...
def _python(t: Table) -> str:
    ...
    out += [
        f"    {v.name!r}: ({v.op}, {v.params!r}, {v.schema}, {v.stream == 'many'}, {v.entry!r}),"
        for v in t.verbs
    ]
    out += ["}", ""]
    return "\n".join(out)
...
def _go(t: Table) -> str:
    out = [
        f"// Code generated by tools/{BANNER} from contract/analytic.toml. DO NOT EDIT.",
        "",
        "package analytic",
        "",
    ...
    out += [
        f'\t{{{v.op}, "{v.name}", "{v.params}", {v.schema}, {str(v.stream == "many").lower()}, "{v.entry}"}},'
        for v in t.verbs
    ]
    out += ["}", ""]
    return "\n".join(out)
...
TARGETS: tuple[tuple[str, str], ...] = (
    ...
    ("bindings/python/irgx/schema.gen.py", "python"),
    ("bindings/rust/src/schema.gen.rs", "rust"),
    ("bindings/go/analytic/schema_gen.go", "go"),
)
...
def main(argv: list[str]) -> int:
    ...
    for rel, lang in TARGETS:
        path, body = ROOT / rel, EMIT[lang](table)
    ...
    if drift:
        print(
            "schema tables are stale — rerun `python3 tools/build_schema_tables.py`:",
            file=sys.stderr,
        )
        for rel in drift:
