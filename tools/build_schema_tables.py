...
decoder is substrate, so the product faces import one rather than each
...
def _rust(t: Table) -> str:
    out = [
        f"//! Generated by tools/{BANNER} from contract/analytic.toml — DO NOT EDIT.",
        "//!",
        "//! The analytic plane's row-schema table. The crate's `decode` walks it",
        ...
        "    pub schema: u32,",
        "    pub many: bool,",
        "    /// The C symbol that answers this verb — one run entry per face",
        "    /// library. Ops are ecosystem-wide, so this is the only thing that",
        "    /// says which library to ask.",
