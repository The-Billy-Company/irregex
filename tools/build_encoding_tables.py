...
these tables live in `src/commands/ripgrep/encoding.zig` (one per WHATWG algorithm).
...
Run: python3 pkg/kernels/gist/tools/build_encoding_tables.py            # writes the .gen.zig
     python3 pkg/kernels/gist/tools/build_encoding_tables.py --check    # diff-only (drift gate)
"""
...
WHATWG = HERE / "whatwg"
OUT = HERE.parent / "src" / "commands" / "ripgrep" / "encoding_tables.gen.zig"
...
def blob(values: list[int], width: int) -> str:
    """A Zig `\\xNN` string literal of the values packed little-endian at `width` bits."""
    nbytes = width // 8
...
def build() -> str:
    lines: list[str] = [
        "//! Code generated by pkg/kernels/gist/tools/build_encoding_tables.py from the",
        "//! pinned WHATWG Encoding Standard indexes in tools/whatwg/. DO NOT EDIT — edit the",
        "//! generator or the vendored indexes and rerun `make gen-gist-encoding`. The drift",
        "//! gate (`make gen-gist-encoding-verify`) re-runs this and diffs the result.",
        "//!",
...
def main() -> int:
    ...
    if "--check" in sys.argv:
        ...
        if current != generated:
            print(f"DRIFT: {OUT} is stale — run `make gen-gist-encoding`", file=sys.stderr)
            return 1
