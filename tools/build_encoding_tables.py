...
these tables live in `src/surface/exec/cold/read/encoding.zig` (one per WHATWG algorithm).
...
Run: python3 pkg/kernels/irregex/tools/build_encoding_tables.py            # writes the .gen.zig
     python3 pkg/kernels/irregex/tools/build_encoding_tables.py --check    # diff-only (drift gate)
"""
...
WHATWG = HERE / "whatwg"
OUT = HERE.parent / "src" / "surface" / "exec" / "cold" / "read" / "encoding_tables.gen.zig"
...
def build() -> str:
    ...
    lines: list[str] = [
        "//! Code generated by pkg/kernels/irregex/tools/build_encoding_tables.py from the",
