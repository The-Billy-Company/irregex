...
Run: python3 tools/build_unicode_tables.py           # writes the .gen.zig
     python3 tools/build_unicode_tables.py --check   # diff-only (drift gate)
"""
...
def build() -> str:
    ...
    lines: list[str] = []
    lines.append("//! Code generated by tools/build_unicode_tables.py from the")
    lines.append(f"//! pinned UCD {UNICODE_VERSION} subset in tools/ucd/. DO NOT EDIT — edit the")
    lines.append("//! generator or the vendored UCD inputs and rerun")
    lines.append("//! `python3 tools/build_unicode_tables.py`.")
    lines.append("//! The drift gate (`python3 tools/build_unicode_tables.py --check`)")
    lines.append("//! re-runs this and diffs the result.")
    lines.append("")
