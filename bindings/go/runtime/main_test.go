...
// binary of its own, so the only producer available was the exact face's.
//
// The tests that genuinely needed a child moved to the repositories that build
// one — the row-and-stats comparison to the exact face's `exact` package, the
// kinship oracle to the kinship package's own bindings — and what remains here
// is the ladder's own reasoning: tier introspection, the FFI escape hatch, seam
// refusals, cancellation, and the digest policy. None of that spawns anything,
// so there is nothing left to certify up front, and the package now runs from a
