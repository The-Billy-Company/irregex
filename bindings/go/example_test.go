...
func ExampleCompileOpts() {
	...
	re := irregex.CompileOpts{Fixed: true}.MustCompile("a.c")
...
func ExampleRegexp_FindAllStringIndex_nullable() {
	fmt.Println(irregex.MustCompile(`a*`).FindAllStringIndex("abc", -1))
	// Output: [[0 1] [2 2]]
}

func ExampleCompile_error() {
	_, err := irregex.Compile(`foo(?=bar)`)
	fmt.Println(err != nil)
	// Lookaround needs the PCRE2 grammar; the default one is linear time.
	re := irregex.CompileOpts{PCRE: true}.MustCompile(`foo(?=bar)`)
