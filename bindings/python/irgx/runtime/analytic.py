...
the roots are not this process's, or the engine returns `IRGX_STALE` — the
...
def verify(ffi: FFI, lib: object) -> None:
    ...
    `irgx_schema_get` lets the failure *name* the schemas that moved instead
    of only reporting that something did.
    """
    theirs = ffi.string(lib.irgx_schema_digest()).decode()
    ...
    msg = (
        ...
        f"Rebuild libirgx and rerun `python3 tools/build_schema_tables.py`."
    )
...
def _drifted(ffi: FFI, lib: object) -> list[str]:
    ...
    # comparison.
    if not all(hasattr(lib, sym) for sym in ("irgx_schema_count", "irgx_schema_get")):
        return []
    count = lib.irgx_schema_count()
    out = ffi.new("irgx_schema *")
    names: list[str] = []
    for sid in range(1, count + 1):
        out.struct_size = ffi.sizeof("irgx_schema")
        if lib.irgx_schema_get(sid, out) != _OK:
...
def _engine(ffi: FFI, lib: object, roots: tuple[str, ...]) -> object | None:
    ...
    with _engines_lock:
        if (cached := _engines.get(key)) is not None:
            return cached
        out = ffi.new("irgx_engine **")
        bufs = [ffi.new("char[]", os.fsencode(r)) for r in roots]
        array = ffi.new("char *[]", bufs) if bufs else ffi.NULL
        if lib.irgx_engine_open(array, len(bufs), out) != _OK:
            return None
        if len(_engines) >= _MAX_ENGINES:
            lib.irgx_engine_close(_engines.pop(next(iter(_engines))))
        _engines[key] = out[0]
        return out[0]
...
def _cursor(ffi: FFI, lib: object, handle: object) -> Rows:
    """Wrap a native `irgx_rows` as decoded `Rows`."""
    live = [handle]

    def pull(cap: int) -> list[Row]:
        if not live:
            return []
        buf = ffi.new("irgx_row[]", cap)
        written = ffi.new("size_t *")
        status = lib.irgx_rows_next_batch(live[0], buf, cap, written)
        if status == _MATCH:
    ...
    def stats() -> Stats:
        ...
        out = ffi.new("irgx_stats *")
        out.struct_size = ffi.sizeof("irgx_stats")
        if lib.irgx_rows_stats(live[0], out) != _OK:
    ...
    def close() -> None:
        if live:
            lib.irgx_rows_close(live.pop())

    return Rows(pull, stats, close=close)


def _row(ffi: FFI, raw: object) -> Row:
    """One `irgx_row` into the transport-neutral IR, copying every borrowed byte."""
...
def _value(ffi: FFI, value: object) -> object:
    """One `irgx_value`, read by its tag. Texts and nested rows are copied out of the arena on the way."""
    match table.Tag(value.tag):
        ...
        case table.Tag.TEXTS:
            texts = ffi.cast("irgx_text *", value.ptr)
            return [_utf8(ffi, texts[i].ptr, texts[i].len) for i in range(value.len)]
        case table.Tag.ROWS:
            nested = ffi.cast("irgx_row *", value.ptr)
