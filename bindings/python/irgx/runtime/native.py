...
`dlopen`s `libgist` (and `libirgx` for the substrate) in ABI mode — no C
...
# Keep the substrate handle alive so RTLD_GLOBAL symbols from libirgx stay
...
def _resolve_lib(stem: str, checkout: str | None = None) -> str | None:
    ...
    `checkout` is the directory that owns the library, when that differs from
    the library's own name. It does for exactly one package: the engine builds
    `libirgx` inside a checkout still called `irregex`. Collapsing the two would
    send the sibling walk hunting a `../irgx` that does not exist — and because
    a miss here returns None rather than raising, the FFI tier would simply go
    dark instead of complaining.
...
def _try_load() -> tuple[FFI, object] | None:
    ...
    eng_path = _resolve_lib("irgx", checkout="irregex")
    ...
    try:
        # The substrate, globally, so irgx_engine_* / irgx_rows_* / schema_*
