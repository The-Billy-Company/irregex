...
the exact face's contract module carries that face's `contract/surface.toml`,
for the same reason its header does: a mirror can only be gated where the canonical file lives,
...
- **`grades.py`** carries the kinship calibration — channels, bands, and
  `grade_of` — mirroring the kinship package's `channel.zig`.
