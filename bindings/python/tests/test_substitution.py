...
def test_a_literal_replacement():
    assert irregex.sub(r"\d+", "N", "a1 b22 c333") == "aN bN cN"
    assert irregex.subn(r"\d+", "N", "a1 b22 c333") == ("aN bN cN", 3)


def test_count_caps_the_replacements_and_leaves_the_rest_alone():
    assert irregex.subn(r"\d", "N", "a1b2c3", count=2) == ("aNbNc3", 2)
    assert irregex.sub(r"\d", "N", "a1b2c3", count=0) == "aNbNcN"


def test_no_match_returns_the_original_text():
    assert irregex.sub(r"z+", "N", "abc") == "abc"
    assert irregex.subn(r"z+", "N", "abc") == ("abc", 0)


def test_numbered_backreferences_in_the_template():
    assert irregex.sub(r"(\w+)@(\w+)", r"\2 at \1", "me@here you@there") == (
        "here at me there at you"
    )
    assert irregex.sub(r"(\w)(\w)", r"\2\1", "abcd") == "badc"
    ...
    assert irregex.sub(r"\d+", r"<\g<0>>", "a1 b22") == "a<1> b<22>"
    assert irregex.sub(r"\d+", r"<\0>", "a1 b22") == "a<\x00> b<\x00>"


def test_named_backreferences_in_the_template():
    assert irregex.sub(r"(?P<w>\w+)", r"[\g<w>]", "hi yo") == "[hi] [yo]"
    # \g<1> is the numbered spelling of the same thing.
    assert irregex.sub(r"(\w+)", r"[\g<1>]", "hi yo") == "[hi] [yo]"
...
def test_a_group_the_match_did_not_enter_contributes_nothing():
    ...
    assert irregex.sub(r"(a)|(b)", r"<\1\2>", "ab") == "<a><b>"


def test_an_escape_that_is_not_a_group_reference():
    assert irregex.sub(r"x", r"a\nb", "x") == "a\nb"
    assert irregex.sub(r"x", r"a\\b", "x") == "a\\b"
    assert irregex.sub(r"x", "100%", "x") == "100%"
...
def test_a_template_naming_a_group_that_does_not_exist_is_refused_up_front():
    ...
    with pytest.raises(irregex.error):
        irregex.sub(r"(a)", r"\2", "zzz")
    with pytest.raises(irregex.error):
        irregex.sub(r"(a)", r"\g<nope>", "zzz")
    with pytest.raises(irregex.error):
        irregex.sub(r"(a)", "\\", "zzz")
...
def test_a_callable_replacement_receives_the_match():
    ...
    assert irregex.sub(r"[a-z]+", upper, "ab 12 cd") == "AB 12 CD"
    assert seen == [(0, 2), (6, 8)]


def test_a_callable_can_use_groups():
    assert irregex.sub(r"(\d+)", lambda m: str(int(m.group(1)) * 2), "a1 b20") == "a2 b40"


def test_substitution_over_bytes_stays_bytes():
    assert irregex.sub(rb"\d+", b"N", b"a1 b22") == b"aN bN"
    assert irregex.sub(rb"(\w)(\w)", rb"\2\1", b"abcd") == b"badc"
    assert isinstance(irregex.sub(rb"x", b"y", b"x"), bytes)
...
def test_substitution_over_non_ascii_text_keeps_the_untouched_parts_intact():
    ...
    assert irregex.sub(r"\d+", "N", text) == "café=N ünïcödé=N naïve=N"
    assert irregex.sub(r"(\w+)=(\d+)", r"\2:\1", text) == "1:café 22:ünïcödé 333:naïve"


def test_expand_renders_a_template_against_one_match():
    match = irregex.search(r"(?P<k>\w+)=(?P<v>\d+)", "answer=42")
    assert match.expand(r"\g<v> is \g<k>") == "42 is answer"
    assert match.expand(r"\2/\1") == "42/answer"


def test_split_on_a_separator():
    assert irregex.split(r"\s*,\s*", "a , b,c") == ["a", "b", "c"]
    assert irregex.split(r",", "a,b,c", maxsplit=1) == ["a", "b,c"]
    assert irregex.split(r"z", "abc") == ["abc"]


def test_split_keeps_declared_groups_the_way_re_does():
    assert irregex.split(r"(\s*)(,)(\s*)", "a , b") == ["a", " ", ",", " ", "b"]


def test_split_reports_empty_leading_and_trailing_fields():
    assert irregex.split(r",", ",a,") == ["", "a", ""]
    assert irregex.split(r",", "") == [""]


def test_split_over_non_ascii_text():
    assert irregex.split(r"·", "café·thé·eau") == ["café", "thé", "eau"]
