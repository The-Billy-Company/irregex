...
is placed by the build hook when a wheel is made. So point ``IRREGEX_LIB`` at
...
_PACKAGE = _TESTS.parent / "irregex"
_ENGINE = _TESTS.parents[2]

_NAMES = ("libirregex.dylib", "libirregex.so", "irregex.dll")


def _point_at_the_engine_build() -> None:
    if os.environ.get("IRREGEX_LIB"):
        return
    ...
    for folder in ("lib", "bin"):
        for name in _NAMES:
            ...
            if candidate.is_file():
                os.environ["IRREGEX_LIB"] = str(candidate)
                return
