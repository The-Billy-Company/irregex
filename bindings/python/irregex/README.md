...
**Iteration comes from `irregex_find_all`.** The engine owns what a match
...
`_abi.py` resolves the library at import: the `IRREGEX_LIB` environment
override if set, otherwise `irregex/lib/libirregex.{dylib,so,dll}` inside the
...
PCRE2 grammar can express comes back as `IRREGEX_STALE` - a declinature, the one
...
string to compare. `IRREGEX_INVALID` is a real failure and becomes `error`,
...
not translate `IRREGEX_STALE`: what to do about a declinature depends on the
