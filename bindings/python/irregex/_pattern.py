...
**Iteration comes from ``irregex_find_all``, never from a loop over
``irregex_captures``.** The engine owns what a match sequence is - whether an
...
per thread. A Python caller will put ``PAT = irregex.compile(...)`` at module
...
def flag_bits(
    *,
    ...
    """The ``IRREGEX_*`` bit word for a set of keyword flags.

    ``unicode`` is inverted on purpose: Unicode semantics are the engine's
    default, so the bit that exists is ``IRREGEX_NO_UNICODE`` and passing
...
class Pattern:
    ...
    Build one with :func:`irregex.compile` rather than by calling this
    directly.
    """
    ...
    def __init__(self, pattern: str | bytes, flags: int) -> None:
        ...
        status = lib.irregex_group_count(handle, ctypes.byref(count))
    ...
    def _resolve_names(self, handle: Any) -> dict[str, int]:
        ...
        for index in range(1, (self._groups or 0) + 1):
            status = check(
                lib.irregex_group_name(handle, index, ctypes.byref(name)),
    ...
    @property
    def flags(self) -> int:
        """The raw ``IRREGEX_*`` bit word this pattern compiled under."""
        return self._flags
    ...
    def __repr__(self) -> str:
        return f"irregex.compile({self._source!r})"
    ...
    def _find_all(self, data: bytes) -> list[tuple[int, int]]:
        ...
        def sweep(window: int) -> tuple[list[tuple[int, int]], int]:
            ...
            check(
                lib.irregex_find_all(handle, data, size, out, window, ctypes.byref(written)),
    ...
    def is_match(self, text: str | bytes) -> bool:
        ...
        status = check(
            lib.irregex_is_match(self._handle(), view.data, len(view.data)),
            f"could not search with {self._source!r}",
            self._source,
        )
