...
doc_radar:
  counts:
    - description: "the eight regex pipeline stages the list below names"
      glob: src/kernel/regex/*/
      unit: dirs
      equals: 8
    - description: "the kernel tiers the toolkit map enumerates"
      glob: src/kernel/*/
      unit: dirs
      equals: 8
    - description: "the corpus packages the toolkit map enumerates"
      glob: src/corpus/*/
      unit: dirs
      equals: 5
    - description: "the persisted index artifacts + substrate the toolkit map enumerates"
      glob: src/corpus/index/*/
      unit: dirs
      equals: 7
    - description: "the two runtimes: one process per query, one resident session"
      glob: src/exec/*/
      unit: dirs
      equals: 2
    - description: "the cold pipeline's concern packages"
      glob: src/exec/cold/*/
      unit: dirs
      equals: 7
  sentinels:
    - description: "the determinizer's two caps the prose quotes"
      file: src/kernel/regex/linear/dfa/powerset.zig
      contains:
        ["pub const max_states: u32 = 4096;", "pub const max_visits: u64 = 750_000;"]
    - description: "there is exactly one grammar, and the ward contract is what makes that a property"
      file: contract/irregex.ward
      contains: "seal kernel/regex through regex.zig"
    - description: "the counted-repetition ceiling the parser enforces"
      file: src/kernel/regex/syntax/parser.zig
      contains: "const max_repeat: usize = 1000;"
    - description: "the predicate alphabet's ceiling on the symbolic road"
      file: src/kernel/regex/linear/symbolic/alphabet.zig
      contains: "pub const max_predicates: u32 = 512;"
    - description: "the engine semver is read from build.zig.zon, never restated, and the vendored grammar version is declared beside it"
      file: src/root.zig
      contains:
        - 'pub const version_string: [:0]const u8 = @import("build_options").version;'
        - 'pub const pcre2_version_string = "10.47";'
    - description: "the pinned UCD the class and fold tables are generated from"
      file: src/kernel/regex/unicode/tables.gen.zig
      contains: 'pub const unicode_version = "16.0.0";'
    - description: "a declined tier is a status, not an error - the whole refusal contract rests on this code"
      file: include/irgx.h
      contains: ["#define IRGX_STALE (-1)", "int32_t irgx_last_fault(irgx_fault *out);"]
    - description: "every transport lowers through one compiled query, so no face can drift on what a hit is"
      file: src/kernel/query/query.zig
      contains: ["pub const CompiledQuery", "error.Unsupported"]
    - description: "both index prunings come off ONE parse, which is a soundness property rather than a tidy one"
      file: src/kernel/query/prefilter.zig
      contains: ["pub const Winnow", "pub fn winnow"]
    - description: "the freshness law's one predicate reads both clocks and treats absence as change"
      file: src/corpus/tree/bulkstat.zig
      contains: "pub fn needsLiveRead"
    - description: "every persisted artifact loads through one framing protocol"
      file: src/corpus/index/frame/frame.zig
      contains: "pub fn mapArtifact"
    - description: "the literal dispatcher's two-authority contract: an exact set decides, a cover only nominates"
      file: src/kernel/scan/literal_set.zig
      contains: "pub const Authority = enum { exact, candidate };"
---
