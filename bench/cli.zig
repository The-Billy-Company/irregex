...
/// Is this a codegen artifact? Same contract the repo's shape gates use
/// (file-length.mdc / rule-of-five.mdc): a known generated filename suffix, or a
/// generated-by marker on the first line. Path-suffix first (no bytes needed,
/// catches the dominant `*.pb.go` flood); the byte sniff catches hand-suffixed
/// outputs (`// Code generated … DO NOT EDIT`). Ranking demotes these — they win
/// lexical+symbol but are never the agent's edit target. Sound to be liberal:
/// a false demote only reorders, never drops, a match.
...
const gen_markers = [_][]const u8{ "// Code generated", "# Generated by", "@generated", "/* Code generated" };
fn isGenerated(path: []const u8, buf: []const u8) bool {
...
fn fileDoc(buf: []const u8, path: []const u8, needle: []const u8, id: u32) ?Doc {
    ...
    const generated = isGenerated(path, buf);
    if (match_lines == 0) {
