...
doc_radar:
  sentinels:
    "bench/scale/artifact/positional_pareto.tsv":
      contains: ["carried_trigrams", "pct_corpus", "cand_pgxpool"]
    "bench/scale/artifact/scale_elision.tsv":
      contains: ["indexed_eq_noindex"]
      absent_matches: ["\\tNO$"]
---

# `bench/scale` — index tiers under load, in Layer D's own unit
...
```bash
python3 bench/scale/scale_race.py --corpus <corpus> --gist-dir <gistdir> \
...
python3 bench/certify/certify_scale_report.py \
  ...
  --race bench/scale/artifact/scale_race.tsv \
  --build bench/scale/artifact/scale_build.tsv \
  --resident bench/scale/artifact/scale_resident.tsv \
  --pareto bench/scale/artifact/positional_pareto.tsv \
  --elision bench/scale/artifact/scale_elision.tsv \
