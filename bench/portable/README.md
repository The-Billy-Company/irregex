...
doc_radar:
  sentinels:
    ...
    - description: "the conformance slate defeats gist's output cap, proves the vendored PCRE2 per target, and caps the translation-layer lane at its own rung"
      file: pkg/kernels/irregex/bench/portable/matrix.py
    ...
    - description: "the Wine ceiling is read by the scorer, not just declared"
      file: pkg/kernels/irregex/bench/portable/portable.py
    ...
    - description: "a sweep is hermetic against the ~10 coworker agents editing this tree"
      file: pkg/kernels/irregex/bench/portable/crossbuild.py
...
# `bench/portable` — the target matrix, executed
...
```bash
python3 bench/portable/portable.py run          # the sweep → artifact/portable.json
python3 bench/portable/portable.py status       # read the last sweep back
python3 bench/portable/portable.py selftest     # offline: probe parity, corpus, objfmt
python3 bench/portable/portable.py run --only s390x-linux-gnu --only riscv64-linux-musl
python3 bench/portable/portable.py run --no-exec   # build + identify only, no Docker
```
...
seam — `kernel/primitives/bits.zig`'s `laneMask`, which reverses the bits inside a
...
```bash
python3 "$HERE/certify_portable_report.py" --certificate "$CERT" --json "$ROOT/bench/portable/artifact/portable.json"
```
