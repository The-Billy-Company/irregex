...
doc_radar:
  paths_exist:
    - pkg/kernels/irregex/bench/compose/bench.zig
    - pkg/kernels/irregex/src/kernel/match/regex/linear/compose/compose.zig
  sentinels:
    - description: "the harness fails closed on disagreement and on the gate arming where it must not"
      file: pkg/kernels/irregex/bench/compose/bench.zig
      contains:
...
# bench/compose — the composition rung's production proof harness
