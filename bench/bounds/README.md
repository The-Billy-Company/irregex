...
certificate, plus a pointer to Layer F, which lives with `relate`. Where the
sibling `gist` repo's `dominance/` measures the product against rivals, this
bucket measures the engine against *physics and information theory*: the
memory roof, the candidate-byte floor, and the static/measured instruction
budget.
...
- **`relate/bench/bounds/codex/`** (sibling `relate` repo) is the self-index
  against its own order-0 entropy bound — `scale.zig` plus `race.sh`, which
  drives Layer F.
