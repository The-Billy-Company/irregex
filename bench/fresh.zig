...
fn widen(gpa: std.mem.Allocator, paths: *std.ArrayList([]const u8), ids: *std.ArrayList(u32), fresh: []const []const u8) !void {
...
test "widen: new file is appended + id'd; existing fresh file is forced once" {
    ...
    try widen(gpa, &paths, &ids, &.{ "a/y.zig", "a/z.zig", "a/y.zig" });
