...
- machine: **Apple M4 Max** · zig `0.16.0` · corpus 1241 files · 22.4 MiB
...
|---|---|--:|--:|--:|--:|--:|
| hex-8  (uuid/sha) | `[0-9a-f]{8}` | 87.1% | 0.1% | 1.9 | 0.7 | 2.9x |
| hex-12 (mac/hash) | `[0-9a-f]{12}` | 91.9% | 0.2% | 2.0 | 0.4 | 5.0x |
| digit-4 (year) | `[0-9]{4}` | 61.6% | 13.9% | 0.7 | 0.3 | 2.3x |
| digit-6 | `[0-9]{6}` | 88.6% | 17.2% | 1.3 | 0.3 | 3.9x |
| upper-4 (CONST) | `[A-Z]{4}` | 12.2% | 2.1% | 0.1 | 0.1 | 1.4x |
| upper-6 | `[A-Z]{6}` | 24.5% | 2.6% | 0.4 | 0.2 | 1.9x |
| ci-hex-8  (?i)uuid | `[0-9a-f]{8}` | 87.1% | 0.1% | 2.2 | 0.5 | 4.0x |
| ci-hex-12 (?i)mac | `[0-9a-f]{12}` | 91.9% | 0.2% | 2.7 | 0.6 | 4.2x |
| ci-upper-6 -iu A-Z | `[A-Z]{6}` | 0.0% | 0.0% | 0.0 | 0.0 | 0.8x |
| word-3 (wide) | `\w{3,8}` | 0.0% | 0.0% | 0.0 | 0.0 | 0.8x |
| alpha-5 (wide) | `[A-Za-z]{5}` | 0.0% | 0.0% | 0.0 | 0.0 | 0.8x |
| alt hex-12|rule-60 | `[0-9a-f]{12}|~{60}` | 81.9% | 0.2% | 6.7 | 3.6 | 1.8x |
| alt digit-6|CONST-6 | `[0-9]{6}|[A-Z]{6}` | 22.2% | 1.5% | 0.4 | 0.3 | 1.5x |
| \d{6}  -u default | `\d{6}` | 74.5% | 10.0% | 3.7 | 1.5 | 2.4x |
| \d{4}  -u default | `\d{4}` | 53.7% | 6.4% | 1.8 | 0.8 | 2.3x |
| \w{8}  -u (wide) | `\w{8}` | 0.1% | 0.0% | 0.0 | 0.0 | 0.9x |
| \s{4}  -u | `\s{4}` | 14.3% | 0.0% | 1.4 | 1.2 | 1.2x |
| [0-9]{6} -u (twin) | `[0-9]{6}` | 88.6% | 17.2% | 1.2 | 0.3 | 3.7x |

**Narrow class-repetition slate (14 patterns): the crest sieve prunes a geomean 52% of files the trigram index prunes 0% on, for a **2.5× geomean end-to-end speedup** — while the count-cousin at the same ĝ prunes only 1.4%, the gap that proves the run is the necessary condition.**
