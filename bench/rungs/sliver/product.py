...
that can exercise it end to end is `gist` — and `gist` moved to its own
checkout when the kernel split into four. A path pinned under this repo's
...
Resolution order is tightest pin first. `GIST_BIN` names an exact build, which
is what a mint wants: a committed number should be traceable to the bytes that
produced it. Failing that, the sibling checkout's release build beside this one
— the binary you just built is the binary you meant. Failing that, whatever is
installed, so an ad-hoc run works without ceremony.
"""
...
@functools.cache
def gist_cli() -> str:
    """The `gist` executable these lanes drive, or a diagnosis of why there is none.
