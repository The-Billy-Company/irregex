...
| Option | Blast radius | Subvertible by an agent? | Friction |
|---|---|---|---|
| **Unprivileged `thread_selfcounts`** (shipped, default) | none — no privilege exists to abuse | no | none |
| **Blessing helper + digest-pinned NOPASSWD** (staged here) | ktrace/kperf ownership for one process: a timing side channel + sampling facility. Not root. | no, while the install path stays root-owned — see below | one authenticated install; re-install to change the helper |
| NOPASSWD on the benchmark binary | **full root**, permanently | **yes, trivially** — any agent writes `zig-out/` | none |
| `timestamp_timeout` extended | full root for the window, for *any* command | yes — any agent can ride the live timestamp | one password per session |
| Entitlements / code signing | n/a | n/a | **not available**: on release kernels `_current_task_can_own_ktrace()` requires euid 0 outright; the entitlement path is dev/debug-kernel only, so no provisioning profile, paid or otherwise, unlocks this |
...
| Cannot bless | **warn and run anyway**, unprivileged. A benchmark must not fail to run; it just measures via `thread_selfcounts`. |
...
| Path | What |
|---|---|
| `pmu_bless.c` | the helper. ~250 lines, commented to be audited in one sitting |
| `kperf_probe.c` | answers "can this process read counters?" — used to verify the grant, and to prove the unprivileged baseline is DENIED |
| `irregex-pmu.sudoers.in` | the sudoers template. Placeholders are deliberately invalid syntax, so a stray `cp` cannot become a live rule |
| `install.sh` | compile → self-check → render → `visudo -c` → install → **verify → revoke on failure** |
