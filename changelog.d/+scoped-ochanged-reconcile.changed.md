...
feeds an exact dirty-path log (`src/runtime/session/dirty.zig`: bounded, deduped,
...
walk's own `Ignore` admission rules (`src/runtime/session/delta.zig`: canonical
...
(`src/runtime/session/scoped_test.zig`) asserts scoped answers against an independent
