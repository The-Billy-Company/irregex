...
feeds an exact dirty-path log (`src/gist/session/dirty.zig`: bounded, deduped,
...
walk's own `Ignore` admission rules (`src/gist/session/delta.zig`: canonical
...
(`src/gist/session/scoped_test.zig`) asserts scoped answers against an independent
