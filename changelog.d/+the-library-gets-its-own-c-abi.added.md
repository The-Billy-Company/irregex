`libirregex` is now a real artifact, and it is a regex library rather than a
search one: `irregex_compile` / `is_match` / `find_all` / `captures` /
`group_count` / `group_index` over a buffer the host already holds, with
`include/irregex.h` as the normative header. No corpus, no session, no index —
...
codes and their dispositions, `irregex_last_fault`, `irregex_status_message`, and
the pattern flag bits (now including `IRREGEX_PCRE` at bit 8, beside the bits
