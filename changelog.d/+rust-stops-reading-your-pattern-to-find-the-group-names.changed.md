...
and `(?<x>)` and then confirming each candidate through `irregex_group_index`.
...
ABI 2 adds `irregex_group_name`, so the table is now walked out of the handle:
group 1 through `irregex_group_count`, whatever the engine calls each one, the
...
`irregex_fault` now says which string its offset is measured in instead of
...
`ABI_VERSION` is 2 and still checked before the first call, so a `libirregex`
handed in through `IRREGEX_LIB_DIR` that speaks ABI 1 says so in a sentence
