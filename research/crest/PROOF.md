...
**One sentence.** Index each document by the *vector of its longest
consecutive runs per byte-class* (its **crest vector**), derive from any regex
the *vector of runs it is forced to contain* (its **forced crest**) via a
...
The name: the maximal class-run is the *crest* — the peak height — of that
...
**required substrings** and tests substring *presence*:

- Cox, *Regular Expression Matching with a Trigram Index* (2012) — required
...
- Zoekt, GitHub Blackbird, and the recent n-gram-selection literature
  (REI SIGMOD'25, the VLDB'25 selection study) — all n-gram *presence*
...
the *matcher* cheap on `C{n,m}`; they build **no per-document index** and prune
**no documents**. Run-length structures in the string-index literature
(SBC-tree, RLE-BWT long-match) index runs of *one repeated character* to answer
*substring* queries — not a per-document max-run-**per-class** vector used as a
regex necessary condition.

> **The object that is new:** a per-document signature `ρ(d) ∈ ℕ^k` of maximal
> *consecutive-class-run* lengths over a fixed class lattice, paired with a
> sound lower-bound *forced-crest* functional `ĝ(R,·) ∈ ℕ^k` extracted from
...
> 2026-07-19, verdict **NOVEL** — see the spike dossier trail): the *composite*
...
complementary — literals still win where they exist); it is a *filter*, never a
...
*Proof.* Suppose `R` matches a substring of `d`: there is `w ∈ L(R)` occurring
...
if** the class lattice and the matcher decide over the *same* alphabet. A
...
a false negative of the *instantiation*, not of the theorem.
...
### 3.7 Why the *run*, not the *count* (the weaker cousin, ruled out)
...
gap is decisive because source files carry hundreds of *scattered* digits but
rarely a *consecutive* run of 8 hex bytes: measured on the live corpus
...
*Proof.* Structural induction over the AST.

*Base.* A mandatory byte from `B`: if `B ⊆ C` every accepted `w` is a single
...
*Concatenation.* Any `w ∈ L(E)` factors as `w = w₁w₂`, `wᵢ ∈ L(Eᵢ)`. Its
longest C-run either lies inside one factor (length `≥ F(Eᵢ)` by IH) or
crosses the seam — in which case it *contains* the trailing run of `w₁` abutted
...
*Alternation.* `L(E) = L(E₁) ∪ L(E₂)`: a min over a union is the min of the
mins, and each componentwise min of lower bounds lower-bounds it. `all_in`
must survive both branches.

*Repetition.* Reduces to `n`-fold concatenation (adversary empties optional
...
fragment) prunes essentially every file lacking a *genuine* ≥8 hex structure.
...
- **soundness, fail-closed** — for *every* file × *every* query:
...
are *realized product speedups*, not model numbers.
